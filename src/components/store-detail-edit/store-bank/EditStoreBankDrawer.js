...
import { Form, Row, Col, Button, Input, Drawer, message, Select, Icon } from 'antd';
...
class EditStoreBankDrawer extends Component {
  state = {
    bankList: [
      "ธนาคารกรุงเทพ",
      "ธนาคารกสิกรไทย",
  ...
  handleSubmit = e => {
    e.preventDefault();
    const { bankDetail, form, handleClickSave } = this.props
    form.validateFields(async (err, values) => {
      if (!err) {
        console.log(values);
        if (bankDetail) {
          handleClickSave({ ...values, bank_id: bankDetail.bank_id })
        } else {
  ...
  componentDidMount = () => {
    const { bankDetail, form } = this.props
    if (bankDetail) {
      const {
        bank_name,
        account_name,
...
export default Form.create({ name: "edit_store_bank_drawer_Form" })(EditStoreBankDrawer)